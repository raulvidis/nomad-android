...
private fun DashboardContent(
    ...
    val quickAccessItems = remember(data.contentPackCount, data.aiStatus, data.storageMetrics) {
        val mapSubtitle = "${data.contentPackCount} packs loaded"
        val archiveSubtitle = data.storageMetrics?.let {
            "%.1f GB".format(it.usedBytes / 1_000_000_000.0)
        } ?: "N/A"
        val aiSubtitle = data.aiStatus?.modelName ?: "N/A"
        listOf(
            QuickAccessItem("OFFLINE MAPS", mapSubtitle, Routes.MAPS),
            QuickAccessItem("ARCHIVES", archiveSubtitle, Routes.KNOWLEDGE),
            QuickAccessItem("AI TERMINAL", aiSubtitle, Routes.CHAT),
