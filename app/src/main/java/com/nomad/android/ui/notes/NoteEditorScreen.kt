...
private fun MarkdownText(content: String) {
    val greenStyle = SpanStyle(
        color = PhosphorGreen,
        fontFamily = FontFamily(Font(R.font.space_grotesk_regular, FontWeight.Normal)),
        fontSize = 14.sp,
    )
...
private fun AnnotatedString.Builder.appendMarkdownInline(text: String, baseStyle: SpanStyle) {
    ...
    allMatches.sortedBy { it.first.first }.forEach { (range, style) ->
        ...
        val contentStart = text.indexOf(':', range.first) + 1
        val innerText = text.substring(range.first + 2, range.last)
